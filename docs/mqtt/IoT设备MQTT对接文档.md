...
> 2026-08-28：四代机 P4/C5 改为**独立升级**（各自版本号、各自发布），报文格式不变，
> 设备端须逐芯片比对版本；作废 08-24 的"两颗共用一个版本号"约定。
...
> **版本号约定（2026-08-28 修订，务必注意）**：P4/C5 **各自独立升级、独立版本号**，
> `p4.version` 与 `c5.version` **可能不同**，设备端必须**逐芯片分别比对**，
> 不可用其中一颗的版本代表整机版本。
> （2026-08-24 曾短暂约定"两颗共用一个版本号"，该约定已作废。）
...
- 收到响应后**逐芯片比对 version**：两颗芯片版本号相互独立，必须分别比对，
  只升级不一致的那颗，另一颗不受影响。
- 下载完成后校验 md5（有值才校验），不一致则丢弃不刷写。
- **无更新的表达**：某颗芯片当前无可用固件时，响应里**不返回该芯片的 key**（设备视为该芯片无更新）；
  两颗都没有则 `bizData` 为空对象。

#### 云端数据源

固件由 PC 端「设备固件升级管理」维护（`dye_firmware_upgrade`）：**一行 = 一颗芯片的一个固件包**
（`chip` 字段区分 esp32p4/esp32c5），云端按芯片各取"已上架的最新一条"，两颗芯片可分别发布、分别停用。
